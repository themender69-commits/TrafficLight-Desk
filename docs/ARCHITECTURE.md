...
- 复制 `agent-hooks/` 下脚本到各工具配置目录（**同一源码，不同 adapter 注册事件名**）
...
**Cursor Hook 事件：** `beforeSubmitPrompt`、`postToolUse`、`preToolUse`、`beforeShellExecution`、`beforeMCPExecution`、`postToolUseFailure`、`subagentStart`、`stop`、`sessionEnd`

**Claude Hook 事件：** `UserPromptSubmit`、`PostToolUse`、`PreToolUse`、`Notification`、`PermissionRequest`、`Stop`、`SessionEnd`

源码目录 **`agent-hooks/`** 为三者共享；`desktop/paths.cjs` 解析开发与打包路径。
...
## Hook 状态机（`agent-hooks/tl-state.sh`）
...
打包后 Hook 脚本路径由 `desktop/paths.cjs` 解析（`extraResources` 中的 `agent-hooks/`）。
